...
//  GPIO_PinLockConfig(GPIOE, GPIO_Pin_9 | GPIO_Pin_11 | GPIO_Pin_13);
 ...
 // GPIO_Init(GPIOE, &GPIO_InitStructure);  
  ...
  ADC_InjectedChannelConfig(ADC2, PHASE_A_ADC_CHANNEL, 1, 
                                                      SAMPLING_TIME_CK);//��λA ADC
  ADC_InjectedChannelConfig(ADC2, TEMP_FDBK_CHANNEL, 2,
                                                      SAMPLING_TIME_CK);//�¶ȷ���
	//ADC_InjectedChannelConfig(ADC2, REF_VOLT_FDBK_CHANNEL, 3,
  //                                                    SAMPLING_TIME_CK);//�¶ȷ���
  ADC_ExternalTrigInjectedConvConfig(ADC2, ADC_ExternalTrigInjecConv_T1_TRGO); //trgo ���� Trgo ��ʲô
  ...
  NVIC_InitStructure.NVIC_IRQChannel = ADC1_2_IRQChannel;//ADC1��ADC2 ע���ж�
...
void SVPWM_3ShuntCurrentReadingCalibration(void)
{
  ...
  for(bIndex=0; bIndex <NB_CONVERSIONS; bIndex++)//�õ��ص�ѹ
  {
...
void SVPWM_InjectedConvConfig(void)
{
 ...
 //                                                  3, SAMPLING_TIME_CK);
  ...
  ADC_ExternalTrigInjectedConvConfig(ADC1, ADC_ExternalTrigInjecConv_T1_TRGO); //trgo ���� Trgo ��ʲô
  

	ADC_ExternalTrigInjectedConvCmd(ADC1,ENABLE);
  
  /* Bus voltage protection initialization*/                            
  //ADC_AnalogWatchdogCmd(ADC1,ADC_AnalogWatchdog_SingleInjecEnable);//��ѹ���ߣ����Ź���λ
  
  //ADC_AnalogWatchdogSingleChannelConfig(ADC1,BUS_VOLT_FDBK_CHANNEL);
  //ADC_AnalogWatchdogThresholdsConfig(ADC1, OVERVOLTAGE_THRESHOLD>>3,0x00);//���ø�ֵ
